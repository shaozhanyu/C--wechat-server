...
int CSocketTCP::IOCP_Run()
{
	...
	int* pnThreadNo = NULL;
	//IOCPContextKey* pIOCPContext = new  IOCPContextKey; //类指针必须new，因为没有实例化，而对象是直接实例化到内存的
	...
	while (true)
	{
		...
		if (TRUE == nRetCode && 0 == dwBytesTransfered  && NULL != pVoidContextKey)//代表客戶端斷開
		{
			//client close tcp
		...
		if (TRUE == nRetCode && lpOverlapped && pVoidContextKey )//成功的操作
		{
